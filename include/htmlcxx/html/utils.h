...
namespace htmlcxx {
	...
	namespace HTML {
		...
		HTMLCXX_API bool detect_utf8(const char *buf, int size);

		/** Trim the argument string at both end and convert sibling blanks
		 * into a single space */
		HTMLCXX_API std::string single_blank(const std::string& str);

		/** Remove html comments from the string */
		HTMLCXX_API std::string strip_comments(const std::string& str);
		
		/** Convert html entities into their ISO8859-1 equivalents */
		HTMLCXX_API std::string decode_entities(const std::string& str);
		
		/** Get the value of an attribute inside the text of a tag 
		  * @return empty string if the attribute is not found, or the value of 
 		  * the attribute
		  */
		HTMLCXX_API std::string get_attribute(const std::string& tag, const std::string& attr);
